...
def extract_sequences_from_pdf(pdf_path):
    ...
    Usage: extract_sequences_from_pdf.py [-h] pdf_folder
...
def main():
    parser = argparse.ArgumentParser(description="Extract DNA sequences from PDFs in a folder.")
    parser.add_argument("pdf_folder", type=str, help="Path to folder containing PDF files")
    args = parser.parse_args()

    all_sequences = []

    for pdf_file in Path(args.pdf_folder).glob("*.pdf"):
